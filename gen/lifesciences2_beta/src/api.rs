...
/// ```
#[derive(Clone)]
pub struct CloudLifeSciences<S> {
...
impl<'a, S> client::Hub for CloudLifeSciences<S> {}

impl<'a, S> CloudLifeSciences<S> {

    pub fn new<A: 'static + client::GetToken>(client: hyper::Client<S, hyper::body::Body>, auth: A) -> CloudLifeSciences<S> {
        CloudLifeSciences {
            client,
            auth: Box::new(auth),
...
impl<'a, S> ProjectMethods<'a, S> {
    ...
    pub fn locations_pipelines_run(&self, request: RunPipelineRequest, parent: &str) -> ProjectLocationPipelineRunCall<'a, S> {
    ...
    S::Error: Into<Box<dyn StdError + Send + Sync>>,
{
    ...
    /// Even though the property as already been set when instantiating this call,
    /// we provide this method for API completeness.
    pub fn request(mut self, new_value: RunPipelineRequest) -> ProjectLocationPipelineRunCall<'a, S> {
        self._request = new_value;
        self
    }
    ...
    /// Even though the property as already been set when instantiating this call,
    /// we provide this method for API completeness.
    pub fn parent(mut self, new_value: &str) -> ProjectLocationPipelineRunCall<'a, S> {
